...
(function () {
  ...
  function computeRoundRatings(round) {
    ...
    teamIds.forEach(teamId => {
      ratings[teamId] = 0;
      const teamQuestions = (round.questions?.[teamId] || []).slice(0, questionsCount);
      for (let qIdx = 0; qIdx < questionsCount; qIdx++) {
        ...
        teamIds.forEach(otherId => {
          ...
          const otherQuestions = (round.questions?.[otherId] || []).slice(0, questionsCount);
  ...
  function toLocalQuestionNumber(round, absoluteQuestionNumber) {
    const base = getRoundQuestionBase(round);
    const abs = Math.max(base + 1, parseInt(absoluteQuestionNumber, 10) || (base + 1));
    return abs - base;
  }
  ...
  function toggleSection(sectionId) {
    ...
    if (btn) btn.textContent = section.classList.contains('collapsed') ? '+' : '−';
  }
  ...
  function addTeam() {
    ...
    const color = (colorInput?.value || '#3b82f6').trim();
    ...
    if (state.teams.some(t => (t.name || '').trim().toLowerCase() === nameLower)) {
  ...
  function updateTeamName(teamId, newValue) {
    ...
    if (state.teams.some(t => t.id !== teamId && (t.name || '').trim().toLowerCase() === newNameLower)) {
      showWarning('Команда с таким названием уже существует');
      render();
      return;
    }
  ...
  function ensureGameRoundsFromUI() {
    ...
    if (Number.isFinite(value) && value >= 1 && value <= 50) {
      ...
      input.value = String(state.settings.gameRounds || 5);
    }
  }
  ...
  function startGame() {
    ...
    const totalRounds = state.settings.gameRounds || 5;
    ...
    scheduleSave();
    render();
    showWarning(`Игра начата: ${totalRounds} туров × ${state.settings.questionsPerRound || 12} вопросов`, 'success');
  ...
  function startRound() {
    ...
    if (isGameMode()) {
      ...
      if (progress && (state.completedRounds || []).length >= progress.totalRounds) {
  ...
  function finalizeRound() {
    ...
    if (isGameMode()) {
      ...
      if (completedCount >= totalRounds) {
        showWarning(`Тур ${finalized.index} завершён. Игра окончена!`, 'success');
        state.game = null;
        scheduleSave();
        render();
        return;
      ...
      showWarning(`Тур ${finalized.index} завершён! Начат тур ${finalized.index + 1}.`, 'success');
      startRound();
      return;
    }
  ...
  function renderTeams() {
    ...
    if (!state.teams.length) {
      container.innerHTML = '<p class="empty-state">Нет команд. Добавьте первую команду выше.</p>';
      return;
    }
        ...
        return `
          <div class="player-item ${team.active ? '' : 'inactive'}">
            <div class="player-info">
              <input class="player-name" type="text" value="${escapeHtml(team.name)}" onblur="updateTeamName('${team.id}', this.value)" onkeydown="teamNameKeydown(event, '${team.id}')" ${team.active ? '' : 'disabled'} />
              <input type="color" class="team-color-picker" value="${team.color || '#3b82f6'}" onchange="updateTeamColor('${team.id}', this.value)" title="Цвет команды">
              ${!team.active ? '<span class="inactive-badge">Inactive</span>' : ''}
            </div>
            <div class="player-actions">
              <button onclick="toggleTeamActive('${team.id}')" class="btn-small">${team.active ? '✓' : '○'}</button>
              <button onclick="deleteTeam('${team.id}')" class="btn-small btn-danger">×</button>
            </div>
          </div>
        `;
      })
  ...
  function renderCurrentRound() {
    ...
    if (titleEl) {
      if (isEditingRound()) {
        ...
        titleEl.textContent = editingRound ? `Редактирование тура ${editingRound.index}` : 'Редактирование тура';
    ...
    if (!state.currentRound) {
      container.innerHTML = '<p class="empty-state">Нет активного тура. Нажмите «Начать тур».</p>';
    ...
    if (finalizeBtn) finalizeBtn.style.display = !isEditingRound() ? 'inline-block' : 'none';
    if (saveEditedBtn) saveEditedBtn.style.display = isEditingRound() ? 'inline-block' : 'none';
    if (cancelEditBtn) cancelEditBtn.style.display = isEditingRound() ? 'inline-block' : 'none';
    if (timerBtn) timerBtn.style.display = !isEditingRound() ? 'inline-block' : 'none';

    const round = state.currentRound;
    const teams = (round.teamIds || []).map(id => ({ id, ...getTeamDisplay(id, round) }));
        ...
        <table class="scoring-table">
          <thead>
            <tr>
              ...
              ${Array.from({ length: questionsCount }, (_, i) => {
                ...
                return `<th class="question-header ${isSelected ? 'highlighted-question' : ''}" onclick="selectQuestionForTimer(${absQ})" title="Выбрать вопрос для таймера">Q${absQ}</th>`;
              }).join('')}
              <th>Total</th>
            </tr>
          </thead>
          <tbody>
            ${teams.map((team, teamIndex) => {
              ...
              const rowStyle = team.color ? `style="--team-color: ${team.color}; background-color: ${team.color}20;"` : '';
              const rowClass = team.color ? '' : `team-row-${teamIndex + 1}`;

              return `
                <tr class="${rowClass}" ${rowStyle}>
                  <td class="team-name"><strong>${escapeHtml(team.name)}</strong></td>
                  ${Array.from({ length: questionsCount }, (_, i) => {
                    const qNum = i + 1;
                    const absQ = base + qNum;
                    const isSelected = qNum === (state.currentQuestionNumber || 1);
                    return `
                      <td class="question-cell ${isSelected ? 'highlighted-question' : ''}" data-question="${absQ}">
                        <input type="checkbox"
                          ${qs[i] ? 'checked' : ''}
  ...
  function selectQuestionForTimer(questionNumber) {
    ...
    const questionsCount = round.questionsCount || (state.settings.questionsPerRound || 12);
    const local = Math.max(1, Math.min(questionsCount, toLocalQuestionNumber(round, questionNumber)));
        ...
        const tableHtml = `
          <table class="mini-round-table">
            <thead>
              <tr>
                <th>Команда</th>
                ${Array.from({ length: questionsCount }, (_, i) => `<th>${base + i + 1}</th>`).join('')}
                <th>Σ</th>
                <th>R</th>
              </tr>
            </thead>
            <tbody>
              ${teams
                .map(team => {
                  const qs = (round.questions?.[team.id] || []).slice(0, questionsCount);
                  const score = round.scores?.[team.id] ?? qs.filter(Boolean).length;
                  const rating = round.ratings?.[team.id] ?? 0;
                  const nameStyle = team.color ? `style="color:${team.color}"` : '';
                  return `
                    <tr>
                      <td><strong ${nameStyle}>${escapeHtml(team.name)}</strong></td>
                      ${Array.from({ length: questionsCount }, (_, i) => `<td class="q-cell ${qs[i] ? 'q-correct' : ''}">${qs[i] ? '✓' : ''}</td>`).join('')}
  ...
  function renderLeaderboard() {
    ...
    const rows = Array.from(teamStats.values())
      ...
      .sort((a, b) => b.totalScore - a.totalScore || b.rating - a.rating || b.avgPerRound - a.avgPerRound || a.name.localeCompare(b.name));

    if (!rows.length) {
      container.innerHTML = '<p class="empty-state">Нет результатов. Завершите тур, чтобы увидеть таблицу.</p>';
      return;
    }
    ...
    container.innerHTML = `
      <table class="leaderboard-table">
        ...
        <tbody>
          ${rows.map((t, idx) => {
            const nameCellStyle = t.color ? `style="border-left: 4px solid ${t.color}; padding-left: 10px;"` : '';
            return `
              <tr>
                <td class="rank">${idx + 1}</td>
                <td ${nameCellStyle}><a href="#" class="player-link" onclick="showTeamStats('${t.id}'); return false;"><strong>${escapeHtml(t.name)}</strong></a></td>
                <td>${t.totalScore}</td>
  ...
  function render() {
    ...
    if (gInput) gInput.value = String(state.settings.gameRounds || 5);
  ...
  function renderGameInfo() {
    ...
    const pct = progress.totalRounds ? (progress.currentRound / progress.totalRounds) * 100 : 0;
    gameInfo.innerHTML = `
  ...
  function showTeamStats(teamId) {
    const rounds = (state.completedRounds || []).filter(r => (r.teamIds || []).includes(teamId));
    ...
    const roundsHtml = rounds
      ...
      .map(r => {
        const questionsCount = r.questionsCount || state.settings.questionsPerRound || 12;
  ...
  function editRoundFromHistory(roundId) {
    ...
    scheduleSave();
    render();
    showWarning('Редактирование тура: внесите изменения и нажмите «Сохранить изменения».', 'info');
  }
  ...
  function saveEditedRound() {
    ...
    const idx = (state.completedRounds || []).findIndex(r => r.id === state.editingRoundId);
    if (idx !== -1) {
    ...
    if (state.suspendedRoundState?.currentRound) {
      ...
      state.currentQuestionNumber = state.suspendedRoundState.currentQuestionNumber || 1;
  ...
  function cancelEditRound() {
    ...
    if (state.undoSnapshot?.round) {
      const idx = (state.completedRounds || []).findIndex(r => r.id === state.undoSnapshot.round.id);
      if (idx !== -1) {
  ...
  function exportGameTsv() {
    const rounds = (state.completedRounds || []).slice().sort((a, b) => (a.index || 0) - (b.index || 0));
    if (!rounds.length) {
    ...
    teamIds.forEach(teamId => {
      const firstRound = rounds.find(r => (r.teamIds || []).includes(teamId)) || null;
  ...
  function openQuestionTimer() {
    ...
    if (!state.currentQuestionNumber || state.currentQuestionNumber > questionsCount) {
      state.currentQuestionNumber = 1;
    }

    const absQuestionNumber = toAbsoluteQuestionNumber(round, state.currentQuestionNumber);
        ...
        <div class="timer-modal-content">
          ...
          <div class="timer-settings-panel" id="timerSettingsPanel">
            ...
            <div class="timer-setting">
              ...
              <input type="number" min="0" max="3" value="${timerSettings.decimalDigits}" onchange="updateTimerSetting('decimalDigits', this.value)">
            </div>
            <div class="timer-setting">
              <label>Показать вопросов:</label>
              <input type="number" min="1" max="5" value="${timerSettings.lastQuestionsShown}" onchange="updateTimerSetting('lastQuestionsShown', this.value)">
            </div>
            <div class="timer-setting">
              <label>Основное время (сек):</label>
              <input type="number" min="10" max="300" value="${timerSettings.mainTime}" onchange="updateTimerSetting('mainTime', this.value)">
            </div>
            <div class="timer-setting">
              <label>Доп. время (сек):</label>
              <input type="number" min="5" max="60" value="${timerSettings.additionalTime}" onchange="updateTimerSetting('additionalTime', this.value)">
            </div>
          </div>
  ...
  function renderTimerTeamsTable() {
    ...
    const startAbs = Math.max(minAbs, currentAbs - timerSettings.lastQuestionsShown + 1);
  ...
  function closeQuestionTimer() {
    ...
    if (window.__chgkTimerKeyListener) {
      document.removeEventListener('keydown', window.__chgkTimerKeyListener, true);
  ...
  function nextQuestion() {
    ...
    const questionsCount = state.currentRound.questionsCount || (state.settings.questionsPerRound || 12);
    state.currentQuestionNumber = (state.currentQuestionNumber || 1) + 1;
    if (state.currentQuestionNumber > questionsCount) state.currentQuestionNumber = 1;
  ...
  function bindEvents() {
    ...
    document.getElementById('questionsPerRound')?.addEventListener('change', () => {
      ensureQuestionsPerRoundFromUI();
      renderCurrentRound();
    });

    document.getElementById('gameRoundsInput')?.addEventListener('change', () => {
      ensureGameRoundsFromUI();
      renderGameInfo();
    });
    ...
    document.getElementById('finalizeRoundBtn')?.addEventListener('click', finalizeRound);
    document.getElementById('saveEditedRoundBtn')?.addEventListener('click', saveEditedRound);
    document.getElementById('cancelEditRoundBtn')?.addEventListener('click', cancelEditRound);
    document.getElementById('questionTimerBtn')?.addEventListener('click', openQuestionTimer);

    document.getElementById('exportGameTsvBtn')?.addEventListener('click', exportGameTsv);
    document.getElementById('exportGameTsvTransposeBtn')?.addEventListener('click', exportGameTsvTransposed);

    document.getElementById('exportBtn')?.addEventListener('click', handleExport);
    document.getElementById('importBtn')?.addEventListener('click', handleImport);
    document.getElementById('importFile')?.addEventListener('change', handleImportFile);
    document.getElementById('resetRoundsBtn')?.addEventListener('click', handleResetRounds);
    document.getElementById('resetBtn')?.addEventListener('click', handleResetAll);
