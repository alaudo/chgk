...
function pickDistinctColor(existingColors, seed = Date.now()) {
  ...
  // Golden-ratio hue stepping for distinctness.
...
function getActiveRoundQuestionRange() {
  ...
  const currentRoundIndex = appState.rounds.findIndex(
    (r) => r.id === activeRoundId
  );
  let offset = 0;
  for (let i = 0; i < currentRoundIndex; i++) {
    offset += appState.rounds[i].questionsCount || 12;
  }

  return { offset, questionsCount };
}

function applyQuestionHighlight(questionIndex) {
  const idx = parseInt(questionIndex, 10);
  if (!Number.isFinite(idx) || idx < 1) return;

  document
    .querySelectorAll(".question-cell, .question-header")
    .forEach((cell) => {
      cell.classList.remove("highlighted-question");
    });

  document.querySelectorAll(".question-header").forEach((header, index) => {
    if (index + 1 === idx) header.classList.add("highlighted-question");
  });

  document
    .querySelectorAll(`.question-cell[data-question="${idx}"]`)
    .forEach((cell) => {
      cell.classList.add("highlighted-question");
    });
}

function syncQuestionSelectionFromAbsolute(absQuestionNumber) {
  const range = getActiveRoundQuestionRange();
  if (!range) return;

  const abs = parseInt(absQuestionNumber, 10);
  if (!Number.isFinite(abs) || abs < 1) return;

  const local = abs - range.offset;
  if (local < 1 || local > range.questionsCount) return;

  appState.currentQuestionNumber = local;
  appState.save();

  // If the scoring table is present, update highlight immediately.
  applyQuestionHighlight(local);
}
...
function bindEvents() {
  ...
  // Captain color picker: show suggested distinct color unless user overrides.
  const captainColorInput = document.getElementById("teamColorInput");
  if (captainColorInput) {
    captainColorInput.dataset.userSelected = "false";
    const existingCaptainColors = appState.players
      .filter((p) => p.isCaptain && p.teamColor)
      .map((p) => p.teamColor);
    captainColorInput.value = pickDistinctColor(existingCaptainColors);
    captainColorInput.addEventListener("input", () => {
      captainColorInput.dataset.userSelected = "true";
    });
    captainColorInput.addEventListener("change", () => {
      captainColorInput.dataset.userSelected = "true";
    });
  }
  ...
  document
    .getElementById("allowManualMove")
    ?.addEventListener("change", handleAllowManualMoveChange);
  ...
  document
    ...
    .addEventListener("click", handleReshuffle);
  document
    .getElementById("reshuffleCurrentBtn")
    ?.addEventListener("click", handleReshuffleCurrent);
  document
    .getElementById("addUnassignedPlayersBtn")
    ?.addEventListener("click", handleAddUnassignedPlayers);
  document
  ...
  // View switching
  document
    .getElementById("viewTeamsBtn")
    .addEventListener("click", () => switchView("teams"));
  document
    .getElementById("viewScoringBtn")
    .addEventListener("click", () => switchView("scoring"));

  // Scoring
  document
    .getElementById("finalizeRoundBtn")
    .addEventListener("click", handleFinalizeRound);
  document
    .getElementById("saveEditedRoundBtn")
    ?.addEventListener("click", handleSaveEditedRound);
  document
    .getElementById("cancelEditRoundBtn")
    ?.addEventListener("click", handleCancelEditRound);
  ...
  document
    .getElementById("exportPlayersBtn")
    ?.addEventListener("click", handleExportPlayers);
  document
    .getElementById("importPlayersBtn")
    ?.addEventListener("click", handleImportPlayers);
  document
  ...
  document
    .getElementById("importPlayersFile")
    ?.addEventListener("change", handleImportPlayersFile);

  // Click anywhere inside a question cell toggles the checkbox.
  document
    .getElementById("currentRoundContainer")
  ...
  // Listen for timer.html updates (absolute question number).
  window.addEventListener("storage", (e) => {
    if (e.key !== SHARED_QUESTION_KEY) return;
    const abs = parseSharedQuestionValue(e.newValue);
    if (!abs) return;
    syncQuestionSelectionFromAbsolute(abs);
  });
}
...
function render() {
  ...
  document
    .getElementById("playersTabBtn")
    ?.classList.toggle("active", leaderboardTab === "players");
  document
    .getElementById("teamsTabBtn")
    ?.classList.toggle("active", leaderboardTab === "teams");
}
...
function updateCurrentRoundTitle() {
  ...
  if (appState.editingRoundId) {
    const editingRound = appState.rounds.find(
      (r) => r.id === appState.editingRoundId
    );
    if (editingRound) {
  ...
  if (appState.currentRoundId) {
    ...
    if (progress) {
      ...
      const currentRound = appState.rounds.find(
        (r) => r.id === appState.currentRoundId
      );
      if (currentRound) {
...
function renderPlayers() {
  ...
  if (displayPlayers.length === 0) {
    container.innerHTML =
      '<p class="empty-state">No players added yet. Add your first player above.</p>';
    return;
  }
      ...
      <div class="player-item ${player.active ? "" : "inactive"} ${
        ...
        <div class="player-info">
          ...
          ${
            player.isCaptain
              ? `
            <span class="captain-badge">👑 Captain${
              player.teamName ? ": " + escapeHtml(player.teamName) : ""
            }</span>
        ...
        <div class="player-actions">
          ...
          ${
            player.isCaptain
              ? `<button onclick="editTeamName('${player.id}')" class="btn-small" title="Edit Team Name">🏷️</button>`
              : ""
          }
...
function renderTeams() {
  ...
  if (!appState.currentRoundId) {
    container.innerHTML =
      '<p class="empty-state">No active round. Generate teams to start.</p>';
    ...
    .map((team, teamIndex) => {
      ...
      return `
        <div class="team-card team-card-${teamIndex + 1}">
          <div class="team-header" onclick="pickTeamColor('${
            team.captainId
          }')" title="Выбрать цвет команды" ${
        team.color
          ? `style="background: linear-gradient(135deg, ${team.color}, ${team.color}dd) !important"`
          : ""
      }>
            ...
            ${players
              ...
              <div class="team-member ${
                ...
                ${
                  player.id !== team.captainId
                    ? `
                  <select onchange="movePlayerTo('${player.id}', '${
                        team.id
                      }', this.value)" class="move-select">
          ...
          <div class="team-questions">
            ...
            <div class="team-score-display">
              Score: <strong>${
                questions.filter((q) => q).length
              }/${questionsCount}</strong>
            </div>
          </div>
        </div>
      `;
...
function renderRoundsHistory() {
  ...
  const rounds = appState.rounds.filter(
    (r) =>
      r.id !== appState.currentRoundId && Object.keys(r.scores || {}).length > 0
  );
      ...
      return `
        <div class="round-item">
          <div class="round-header">
            ...
            <button onclick="editRoundFromHistory('${
              round.id
            ...
            ${round.teams
              .map((team) => {
                ...
                <div class="round-team">
                  ...
                  ${
                    hasScores
                      ? `<span class="team-score">${score ?? 0} pts</span>`
                      : ""
                  }
...
function switchLeaderboardTab(tab) {
  ...
  document
    .getElementById("playersTabBtn")
    .classList.toggle("active", tab === "players");
  document
    .getElementById("teamsTabBtn")
    .classList.toggle("active", tab === "teams");
  renderLeaderboard();
}
...
function renderPlayersLeaderboard(container) {
  let leaderboard = appState.computeLeaderboard(
    leaderboardSort.by,
  ...
  if (leaderboard.length === 0) {
    container.innerHTML =
      '<p class="empty-state">No scores yet. Complete a round to see ratings.</p>';
    return;
  }
  ...
  container.innerHTML = `
    <table class="leaderboard-table">
      <thead>
        <tr>
          <th>№</th>
          <th class="sortable" onclick="sortLeaderboard('name')">Игрок ${getSortIcon(
            "name"
          )}</th>
          <th class="sortable" onclick="sortLeaderboard('correctAnswers')">Правильные ${getSortIcon(
            "correctAnswers"
          )}</th>
          <th class="sortable" onclick="sortLeaderboard('avgPerRound')">В среднем/тур ${getSortIcon(
            "avgPerRound"
          )}</th>
          <th class="sortable" onclick="sortLeaderboard('rating')">Рейтинг ${getSortIcon(
            "rating"
          )}</th>
          <th class="sortable" onclick="sortLeaderboard('roundsPlayed')">Туры ${getSortIcon(
            "roundsPlayed"
          )}</th>
          <th class="sortable" onclick="sortLeaderboard('captainRounds')">Капитан ${getSortIcon(
            "captainRounds"
          )}</th>
        </tr>
      </thead>
      <tbody>
        ${leaderboard
          ...
          <tr>
            ...
            <td>
              <a href="#" class="player-link" onclick="showPlayerStats('${
                entry.id
              }'); return false;">
              ...
              ${
                entry.isCaptain
                  ? '<span class="captain-badge-small">👑</span>'
                  : ""
              }
            </td>
...
function renderTeamsLeaderboard(container) {
  ...
  appState.rounds.forEach((round) => {
    ...
    round.teams.forEach((team) => {
      ...
      const teamName =
        captain?.teamName && captain.teamName.trim()
          ? captain.teamName.trim()
          : team.label;
  ...
  const teamsArray = Object.values(teamStats)
    .map((team) => ({
      ...team,
      avgPerRound:
        team.roundsPlayed > 0 ? team.totalScore / team.roundsPlayed : 0,
    }))
    .sort((a, b) => {
  ...
  if (teamsArray.length === 0) {
    container.innerHTML =
      '<p class="empty-state">No team scores yet. Complete a round to see team ratings.</p>';
    return;
  }
    ...
    <table class="leaderboard-table">
      ...
      <tbody>
        ${teamsArray
          ...
          <tr>
            <td class="rank">${index + 1}</td>
            <td><a href="#" class="player-link" onclick="showTeamStats('${encodeURIComponent(
              team.captainId
...
function renderDrawTable(container) {
  ...
  if (rounds.length === 0) {
    container.innerHTML =
      '<p class="empty-state">Нет туров для отображения жеребьёвки. Начните игру или создайте команды.</p>';
    return;
  }
  ...
  const playersInRounds = appState.players.filter((p) =>
    playerIdsInRounds.has(p.id)
  );
  const otherActivePlayers = appState.players.filter(
    (p) => p.active && !playerIdsInRounds.has(p.id)
  );
  ...
  const tableHtml = `
    <div class="draw-table-container">
      ${
        appState.isGameMode()
          ? '<p class="draw-table-note"><strong>📋 Жеребьёвка игры:</strong> Все туры сгенерированы заранее с оптимизацией распределения игроков.</p>'
          : ""
      }
              ...
              <tr>
                ...
                ${rounds
                  .map((round) => {
                    ...
                    const cellClass = team
                      ? team.color
                        ? "team-cell"
                        : `team-cell draw-team-${teamIndex}`
                      : "no-team-cell";
                    const cellStyle =
                      team && team.color
                        ? `style="background-color: ${team.color}30; border-left: 4px solid ${team.color}"`
                        : "";
...
function sortLeaderboard(column) {
  if (leaderboardSort.by === column) {
    leaderboardSort.direction =
      leaderboardSort.direction === "desc" ? "asc" : "desc";
  } else {
...
function showPlayerStats(playerId) {
  ...
  const correctLines = Array.from(correctByRound.entries())
    .map(([roundId, answers]) => {
      ...
      const team =
        round?.teams?.find((t) => t.id === answers[0]?.teamId) ||
        round?.teams?.find((t) => t.playerIds.includes(playerId));
  ...
  const roundsForTogether = appState.isGameMode()
    ? appState.gameRounds
    : appState.rounds;
              ...
              appState.rounds
                ...
                .map((round) => {
                  ...
                  const questionsCount =
                    ...
                    appState.settings.questionsPerRound ||
                    12;
                  ...
                  <table class="mini-round-table">
                    ...
                    <tbody>
                      ${round.teams
                        .map((team) => {
                          const teamQuestions =
                            round.questions?.[team.id] || [];
                          const teamScore = teamQuestions.filter(
                            (q) => q
                          ).length;
                ...
                `;
                  ...
                  const playerTeamQuestions =
                    playerTeam && round.questions?.[playerTeam.id]
                      ? round.questions[playerTeam.id]
                      : [];
                  const playerTeamScore = playerTeamQuestions.filter(
                    (q) => q
                  ).length;
                  ...
                  <div class="round-stat-item">
                    <div class="round-stat-header">
                      <strong>Round ${round.index}</strong> - ${escapeHtml(
                    playerTeam?.label || "Unknown"
                  )}
            ...
            ${
              correctLines.length
                ? correctLines
                    .map(
                      (x) =>
                        `<div class="correct-answer-item">${escapeHtml(
                          x.text
                        )}</div>`
                    )
                    .join("")
...
function showTeamStats(encodedName) {
  ...
  const captainById = appState.players.find(
    (p) => p.isCaptain && p.id === teamKey
  );
  if (captainById) {
  ...
  const captainByName = appState.players.find(
    (p) => p.isCaptain && p.teamName === teamKey
  );
...
function showTeamStatsByCaptainId(captainId) {
  ...
  const firstTeam = roundsWithTeam[0].teams.find(
    (t) => t.captainId === captainId
  );
  const teamName =
    captain.teamName && captain.teamName.trim()
      ? captain.teamName.trim()
      : firstTeam?.label || "Team";

  const roundsHtml = roundsWithTeam
    .map((round) => {
...
function updateActionButtons() {
  ...
  document.getElementById("generateTeamsBtn").disabled =
    !hasCaptains || hasActiveRound || isGameMode;
  document.getElementById("reshuffleBtn").disabled =
    !hasActiveRound || isGameMode;
  ...
  if (finalizeBtn) {
    finalizeBtn.style.display =
      hasActiveRound && !isEditingRound ? "inline-block" : "none";
  ...
  if (reshuffleCurrentBtn) {
    reshuffleCurrentBtn.style.display =
      hasActiveRound && !isEditingRound ? "inline-block" : "none";
  }

  if (addUnassignedBtn) {
    addUnassignedBtn.style.display =
      isGameMode && hasActiveRound && !isEditingRound ? "inline-block" : "none";
  }

  const questionTimerBtn = document.getElementById("questionTimerBtn");
  if (questionTimerBtn) {
    questionTimerBtn.style.display =
      hasActiveRound && !isEditingRound ? "inline-block" : "none";
  }
}
...
function handleAddPlayer() {
  ...
  let teamColor = isCaptain ? teamColorInput.value : null;
  ...
  if (
    appState.players.some((p) => p.name.toLowerCase() === name.toLowerCase())
  ) {
  ...
  if (isCaptain) {
    const existingColors = appState.players
      .filter((p) => p.id !== player.id && p.isCaptain && p.teamColor)
      .map((p) => p.teamColor);

    const userPicked = teamColorInput?.dataset?.userSelected === "true";
    if (!userPicked) {
      teamColor = pickDistinctColor(existingColors);
    }
  ...
  input.value = "";
  teamNameInput.value = "";
  // Prepare next suggested distinct color.
  if (teamColorInput) {
    const allCaptainColors = appState.players
      .filter((p) => p.isCaptain && p.teamColor)
      .map((p) => p.teamColor);
    teamColorInput.value = pickDistinctColor(allCaptainColors);
...
function toggleCaptain(playerId) {
  const player = appState.players.find((p) => p.id === playerId);
  if (player) {
    const nextIsCaptain = !player.isCaptain;
    const updates = { isCaptain: nextIsCaptain };
    if (nextIsCaptain && !player.teamColor) {
      const existingCaptainColors = appState.players
        .filter((p) => p.id !== playerId && p.isCaptain && p.teamColor)
        .map((p) => p.teamColor);
      updates.teamColor = pickDistinctColor(existingCaptainColors);
    }
    appState.updatePlayer(playerId, updates);
    render();
  }
}
  ...
  try {
    ...
    setTimeout(() => {
      try {
        ...
        render();
        showWarning(
          `Игра началась! Сгенерировано ${numRounds} туров с оптимизированными командами.`,
          "success"
        );
      } catch (error) {
...
function handleReshuffle() {
  if (appState.isGameMode()) {
    showWarning(
      "Невозможно перемешать команды в режиме игры. Команды оптимизированы для всех туров."
    );
    return;
  }
...
function handleFinalizeRound() {
  ...
  try {
    ...
    if (wasGameMode && wasLastRound) {
      ...
      showWarning(
        `Тур ${roundNumberBefore}/${totalRounds} завершен! Начат тур ${
          roundNumberBefore + 1
        }/${totalRounds}.`,
        "success"
      );
  ...
  reader.onload = (e) => {
    try {
      ...
      if (added === 0 && skipped > 0 && failed === 0) {
        showWarning(
          `Импорт: добавлять нечего (все ${skipped} уже есть).`,
          "info"
        );
      } else if (failed > 0) {
        showWarning(
          `Импорт игроков: добавлено ${added}, пропущено ${skipped}, ошибок ${failed}.`,
          "error"
        );
      } else {
        showWarning(
          `Импорт игроков: добавлено ${added}, пропущено ${skipped}.`,
          "success"
        );
      }
    } catch (error) {
      showWarning(
        "Ошибка импорта игроков: " + (error.message || error),
        "error"
      );
    }
  };
...
function handleReset() {
  if (
    confirm(
      "Reset all data? This will delete all players, rounds, and scores. This cannot be undone!"
    )
  ) {
    appState.reset();
    render();
    ...
    if (
      ...
      appState.players.some(
        (p) =>
          ...
          p.isCaptain &&
          p.teamName &&
          p.teamName.toLowerCase() === trimmedName.toLowerCase()
      )
    ) {
      showWarning("A team with this name already exists");
      return;
    ...
    <div class="teams-view">
      ${round.teams
        .map((team, teamIndex) => {
          ...
          return `
          <div class="team-card ${
            team.color ? "team-card-custom" : `team-card-${teamIndex + 1}`
          }" ${team.color ? `style="--team-color: ${team.color}"` : ""}>
            <div class="team-header" onclick="pickTeamColor('${
              team.captainId
            }')" title="Выбрать цвет команды" ${
            team.color
            ...
            <div class="team-members">
              ${players
                .map((player) => {
                  const allowMove =
                    document.getElementById("allowManualMove")?.checked;
                  return `
                  <div class="team-member ${
                    player.id === team.captainId ? "captain" : ""
                  }">
...
function renderScoringView() {
        ...
        <thead>
          <tr>
            ...
            ${Array.from(
              { length: questionsCount },
              (_, i) =>
                `<th class="question-header" onclick="highlightQuestionColumn(${
                  i + 1
                })">Q${i + 1}<br><small>(${
                  questionOffset + i + 1
                })</small></th>`
        ...
        <tbody>
          ${round.teams
            .map((team, teamIndex) => {
              ...
              return `
              <tr class="${team.color ? "" : `team-row-${teamIndex + 1}`}" ${
                team.color
  ...
  // Keep highlight in sync with the current question selection.
  applyQuestionHighlight(appState.currentQuestionNumber || 1);
}
...
function handleResetGame() {
  if (
    confirm(
      "Reset the game? This will remove all rounds but keep players. This cannot be undone!"
    )
  ) {
    appState.resetGame();
    render();
...
function toggleTeamNameInput() {
  ...
  if (captainInputs) {
    ...
    if (isCaptain) {
      ...
      const teamColorInput = document.getElementById("teamColorInput");
      if (teamColorInput && teamColorInput.dataset.userSelected !== "true") {
        const existingCaptainColors = appState.players
          .filter((p) => p.isCaptain && p.teamColor)
          .map((p) => p.teamColor);
        teamColorInput.value = pickDistinctColor(existingCaptainColors);
        teamColorInput.dataset.userSelected = "false";
      }
    }
  }
...
function handleAllowManualMoveChange() {
  const allowManualMove =
    document.getElementById("allowManualMove")?.checked || false;
...
function handleAddUnassignedPlayers() {
  ...
  try {
    ...
    const remainingRounds = appState.gameRounds.slice(
      appState.currentGameRoundIndex
    );
    ...
    const unassignedPlayers = nonCaptains.filter(
    ...
    if (
      !confirm(
        `Добавить ${unassignedPlayers.length} неназначенных игроков во все оставшиеся туры?`
      )
    ) {
      return;
    }
    ...
    remainingRounds.forEach((round) => {
      ...
      shuffledPlayers.forEach((player) => {
        ...
        const smallestTeams = teams.filter(
          (t) => (t.playerIds || []).length === minSize
        );
        const target =
    ...
    render();
    showWarning(
      `Добавлено ${unassignedPlayers.length} игроков в оставшиеся туры!`,
      "success"
    );
  } catch (error) {
    showWarning(error.message);
...
function updateTimerDisplay() {
  ...
  if (!displayEl) return;
  const display = Math.max(0, timerSeconds).toFixed(
...
function openQuestionTimer() {
  ...
  if (
    ...
    appState.currentQuestionNumber > questionsCount
  ) {
    appState.currentQuestionNumber = 1;
  }

  const globalQuestionNumber =
    globalQuestionOffset + appState.currentQuestionNumber;
...
function renderTimerTeamsTable() {
  ...
  for (
    let i = Math.max(1, currentQ - timerSettings.lastQuestionsShown + 1);
    i <= currentQ;
    i++
  ) {
  ...
  round.teams.forEach((team) => {
    ...
    questionsToShow.forEach((q) => {
      ...
      html += `<td class="mini-answer mini-answer-clickable" onclick="toggleTimerQuestion('${
        team.id
      }', ${q - 1})">${checked}</td>`;
    });
    html += "</tr>";
...
function highlightQuestionColumn(questionIndex) {
  applyQuestionHighlight(questionIndex);

  // Persist selection and broadcast absolute question number for timer sync.
  appState.currentQuestionNumber = questionIndex;
  appState.save();

  const range = getActiveRoundQuestionRange();
  if (range) {
    publishSharedQuestion(range.offset + (parseInt(questionIndex, 10) || 1));
  }
}
